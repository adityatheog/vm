...
@contextmanager
def db_session():
    """Context manager for SQLite connections to handle commits and prevent database locks."""
...
async def vps_list(ctx, node_id: int = 1):
    ...
    # Resources field text (compact inline stats with progress-like emojis)
    if is_online:
        resources_text = (
    ...
    for i, row in enumerate(rows, 1):
        ...
        status_str = vps.get('status', 'unknown')
        ...
        status_emoji = "🟢" if status_str == 'running' and not suspended_flag else "🟡" if suspended_flag else "🔴"
        vps_status = status_str.upper()
        if suspended_flag:
    ...
    # Resources field with bar visualization
    ...
    # VPS List - chunked embeds with pagination
    if vps_info:
        chunk_size = 6  
        ...
        for idx, chunk in enumerate(chunks[1:], 2):
            ...
            page_embed.set_footer(text=f"Total: {total_vps} VPS | Powered by {BOT_NAME}")
    ...
    for node in nodes:
        ...
        if node['is_local']:
            ...
            try:
                ...
                # Calculate free resources (simplified)
    ...
    if admins:
        ...
        for admin_id in admins:
            try:
                ...
                admin_list_str.append(f"• {admin_user.mention} (ID: {admin_id})")
            except Exception:
                admin_list_str.append(f"• Unknown User (ID: {admin_id})")
        admin_text = "\n".join(admin_list_str)
...
async def user_info(ctx, user: discord.Member):
    ...
    embed.set_footer(text=f"{BOT_NAME} • User Resource Dashboard")
    embed.timestamp = ctx.message.created_at
...
async def server_stats(ctx):
    ...
    embed.set_footer(text=f"{BOT_NAME} • Real-Time Monitoring")
    embed.timestamp = ctx.message.created_at
...
async def node_check(ctx, node_id: int):
    ...
    if status.startswith("🟢"):
        ...
        try:
            requests.get(f"{node['url']}/api/ping", params={'api_key': node['api_key']}, timeout=5)
...
async def node_cmd(ctx, sub: str, *args):
    ...
    # NEW: Add delete subcommand
        ...
        class ConfirmDelete(discord.ui.View):
            def __init__(self, n_id, n_name, f_mode, v_count):
                super().__init__(timeout=60)
                self.node_id = n_id
                self.node_name = n_name
                self.force = f_mode
                self.vps_count = v_count
